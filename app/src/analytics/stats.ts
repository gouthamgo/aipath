...
async function fetchTotalStripeRevenue() {
  ...
  if (!apiKey || apiKey.includes('placeholder') || apiKey.length < 20) {
    console.log('Skipping Stripe revenue fetch: API key not configured');
    return 0;
  }
...
async function fetchTotalLemonSqueezyRevenue() {
  ...
  if (!apiKey || apiKey.includes('placeholder') || apiKey.length < 20) {
    console.log('Skipping Lemon Squeezy revenue fetch: API key not configured');
    return 0;
  }
...
async function fetchTotalPolarRevenue(): Promise<number> {
  // Skip if using placeholder API key
  const apiKey = process.env.POLAR_ORGANIZATION_ACCESS_TOKEN || '';
  if (!apiKey || apiKey.includes('placeholder') || apiKey.length < 20) {
    console.log('Skipping Polar revenue fetch: API key not configured');
    return 0;
  }
